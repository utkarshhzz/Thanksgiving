...
async def create_space_listing(
    ...
    """Host creates a space listing. Starts as DRAFT."""
...
async def browse_spaces(
    ...
    min_capacity: Optional[int] = Query(None, gt=0),
    db: AsyncSession = Depends(get_db),
):
    """Public — browse all ACTIVE space listings."""
    return await list_spaces(db, city, space_type, min_capacity)


# ── IMPORTANT: /mine MUST come BEFORE /{space_id} ──
# FastAPI matches routes top-to-bottom. If /{space_id} is first,
# a request to /mine gets "mine" parsed as a UUID → 422 error.
...
# ── Booking Endpoints — also before /{space_id} ──
...
async def update_space_listing_status(
    ...
    """Host publishes (DRAFT→ACTIVE) or deactivates their space."""
